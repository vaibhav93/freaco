        ...
        if(err || !offer){
            ...
            offer.business(function(err,business){
                if(err || !business){
                    ...
                    business.codes.create({uid:shortid.generate().slice(0,5).toUpperCase(),offerId:body.offerId},function(err,code){
                        if(err){
                            res.statusCode=200;
                            res.send({error:'some error occured'});  
                        } else{
...
Appuser.findOne({ where:query }, function(err,user){
    ...
    if(err){
        ...
        //create user here
        Appuser.create({fname:response.first_name,email:query.email,username:response.id,password:dummy_pass},function(err,user){
            if(err){
                cb(defaultError);
            } else{
                console.log('Appuser created: ' + query.email);
                Appuser.login({email:query.email,password:dummy_pass},function(err,new_accessToken){
